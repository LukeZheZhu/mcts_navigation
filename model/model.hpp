...
namespace nsModel {
    ...
    public:
        ...
        sModelRotation(sModelRotation &rotate) :
            yaw(rotate.yaw),
            roll(rotate.roll),
            pitch(rotate.pitch) {
        };
    ...
    public:
        cModel() :
            m_pModelBase(std::make_shared<cModelBase>) {
        };
        ...
        struct sModelPose calcPose(int index) {
            return m_pModelBase->calcPose(index);
        };
    ...
    private:
        virtual float getRaduis();
        virtual struct sModelPose getPose();
        virtual struct sModelPose calcPose(int index);
    ...
    protected:
        ...
        struct sModelPose calcMove(int index) {
            ...
            m_pose.position.x += m_speed * cos(yaw);
            m_pose.position.y += m_speed * sin(yaw);
